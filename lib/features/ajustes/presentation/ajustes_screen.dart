...
class AjustesScreen extends ConsumerWidget {
  ...
  String _etiquetaDias(int dias) =>
      dias == 0 ? 'El mismo día' : '$dias día(s) antes';
    ...
    return Scaffold(
      ...
      body: ResponsiveContent(
        child: configAsync.when(
          ...
          error: (error, _) =>
              Center(child: Text('Error al cargar ajustes: $error')),
          data: (config) {
            Future<void> actualizar(RecordatoriosConfig nuevo) => ref
                .read(recordatoriosConfigProvider.notifier)
                .actualizar(nuevo);
            ...
            return ListView(
              ...
              children: [
                ...
                SwitchListTile(
                  ...
                  onChanged: (valor) =>
                      actualizar(config.copyWith(citasActivo: valor)),
                ),
                          ...
                          items: _opcionesDias
                              .map(
                                (dias) => DropdownMenuItem(
                                  value: dias,
                                  child: Text(_etiquetaDias(dias)),
                                ),
                              )
                              .toList(),
                          onChanged: (dias) {
                            if (dias != null) {
                              actualizar(config.copyWith(citasDiasAntes: dias));
                            }
                          },
                        ),
                      ),
                          ...
                          items: List.generate(24, (hora) => hora)
                              .map(
                                (hora) => DropdownMenuItem(
                                  value: hora,
                                  child: Text(_etiquetaHora(hora)),
                                ),
                              )
                              .toList(),
                          onChanged: (hora) {
                            if (hora != null) {
                              actualizar(config.copyWith(citasHora: hora));
                            }
                          },
                        ),
                      ),
                ...
                SwitchListTile(
                  ...
                  onChanged: (valor) =>
                      actualizar(config.copyWith(procedimientosActivo: valor)),
                ),
                      ...
                      Expanded(
                        child: DropdownButtonFormField<int>(
                          ...
                          onChanged: (dias) {
                            if (dias != null) {
                              actualizar(
                                config.copyWith(procedimientosDiasAntes: dias),
                              );
                            }
                          },
                        ),
                      ...
                      Expanded(
                        child: DropdownButtonFormField<int>(
                          ...
                          onChanged: (hora) {
                            if (hora != null) {
                              actualizar(
                                config.copyWith(procedimientosHora: hora),
                              );
                            }
                          },
                        ),
...
class _SeccionDatosDespacho extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_SeccionDatosDespacho> createState() =>
      _SeccionDatosDespachoState();
}
...
class _SeccionDatosDespachoState extends ConsumerState<_SeccionDatosDespacho> {
  ...
  Future<void> _guardar() async {
    await ref
        .read(datosDespachoProvider.notifier)
        .actualizar(
          DatosDespacho(
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Datos del despacho guardados')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return datosAsync.when(
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (error, _) =>
          Text('Error al cargar los datos del despacho: $error'),
      data: (datos) {
        ...
        return Column(
          ...
          children: [
            ...
            OutlinedButton(
              onPressed: _guardar,
              child: const Text('Guardar datos del despacho'),
            ),
          ],
        );
      },
